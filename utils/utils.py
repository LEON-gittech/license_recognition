...
def get_latest_run(search_dir='./runs'):
    # Return path to most recent 'yolov5_best.pt' in /runs (i.e. to --resume from)
...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    """Performs Non-Maximum Suppression (NMS) on demo results
...
def print_mutation(hyp, results, bucket=''):
    # Print mutation results to evolve.txt (for use with train_yolov5.py --evolve)
...
def apply_classifier(x, model, modelb, img, im0):  # x是pred
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    STN = STNet()
    STN.to(device)
    STN.load_state_dict(torch.load('weights/stn_green.pth',
                                   map_location=lambda storage, loc: storage)['net_state_dict'])
    STN.eval()
    #############蓝牌
    STNb = STNet()
    STNb.to(device)
    STNb.load_state_dict(
        torch.load('weights/stn-pretrain.pth',
    ...
    for i, d in enumerate(x):  # per image
        ...
        if d is not None and len(d):
            ...
            scale_coords(img.shape[2:], d[:, :4], im0[i].shape)  # 框出图片
            ...
            if(num==1):
                ...
                for j, a in enumerate(d):
                    ...
                    im_transfer = STN_t(images)
                    preds = model_t(im_transfer)
                ...
                for j, a in enumerate(d):  # per item
                    ...
                    # data = torch.from_numpy(im).float().unsqueeze(0).to(device)  # torch.Size([1, 3, 24, 94])
                    im_transfer = STN_t(images)
                    # im_transfer=im_transfer.squeeze(0)
                    preb=model_t(im_transfer)
                    preb = preb.squeeze(0)
                    prebs.append(preb)
                prebs=torch.stack(prebs)
                # prebs=prebs.squeeze(0)
                prebs = prebs.cpu().detach().numpy()
                    #############################
                #     ims.append(im_transfer)
                #     # print(im_transfer)
                # ims = torch.stack(ims)
                # im_transfer = ims.squeeze(0)
                ############################
            # tmp=np.array(ims)
            # print(tmp.shape)
            # rec对截出的框进行识别，在这里可以添加增强图片分辨率的步骤和对歪斜图片进行处理的步骤
            # preds = model(torch.Tensor(im_transfer).to(d.device))  # classifier prediction
            ############################
            # preds = model(im_transfer)
            # prebs = preds.cpu().detach().numpy()
            #############################
            # 对识别结果进行CTC后处理：删除序列中空白位置的字符，删除重复元素的字符
            preb_labels = list()
            for w in range(prebs.shape[0]):  # 每个通道分开看
...
def plot_test_txt():  # from utils.utils import *; plot_test()
    # Plot rec_data.txt histograms
    x = np.loadtxt('rec_data.txt', dtype=np.float32)
    box = xyxy2xywh(x[:, :4])
    cx, cy = box[:, 0], box[:, 1]
...
def plot_study_txt(f='study.txt', x=None):  # from utils.utils import *; plot_study_txt()
    # Plot study.txt generated by test_yolov5.py
